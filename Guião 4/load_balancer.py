...
class RoundRobin:
    ...
    def update(self, *arg):
        # No update logic needed for round-robin selection
        pass
...
class LeastConnections:
    ...
    def select_server(self):
        # Select server with the fewest connections
        server = min(self.connections, key=self.connections.get)
        self.connections[server] += 1
        return server

    def update(self, server):
        # Update connections after a connection is released
...
class LeastResponseTime:
    def __init__(self, servers):
        ...
        self.last_update = {server: time.time() for server in servers}
        self.connections = {server: 0 for server in servers}
        self.small_increment = 1e-10

    def select_server(self):
        # Get server with smallest response time
        server = min(self.response_times, key=self.response_times.get)
        self.last_update[server] = time.time()
        self.response_times[server] += self.small_increment
        return server
    ...
    def update(self, server):
        ...
        self.connections[server] += 1
        now = time.time()
        elapsed_time = now - self.last_update[server]
        self.response_times[server] = (
            (self.response_times[server] * (self.connections[server] - 1) + elapsed_time) / self.connections[server]
        )
